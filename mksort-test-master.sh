		...
		for count in 1 5 10 25 50 100 10000 $((nrows/10)) $nrows; do
			...
			for distribution in random correlated sequential; do
				...
				for ncols in 1 2 3 4 5 6 7 8; do
					...
					echo "EXPLAIN (ANALYZE, TIMING OFF) $query" >> explain.log;
...
EOF
						...
						echo "===== rows $nrows type $dtype count $count distribution $distribution cols $ncols run $r =====" >> explain.log 2>&1
						cat timing.log >> explain.log 2>&1

						echo $nrows $dtype $distribution $count $ndistinct1 $ndistinct2 $ncols $r off $t
